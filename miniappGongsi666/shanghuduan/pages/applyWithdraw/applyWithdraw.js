...
Page({
    data: {
        ...
        flag: true, //控制遮罩层是否显示
        time: 3, //倒计时开始时间
        number: "", //最小提现数量，像100 1000
        string: "", //最小提现数量截取后的字符串
    },
    ...
    onLoad: function (options) {
        this.setData({
            ...
            minMoney: options.minMoney, //最小提现数量
            number: Math.pow(10, Number(options.minMoney.length - 1)),
        });
        this.setData({
            string: (this.data.number + "").substring(
                1,
                this.data.minMoney.length
            ),
        });
    ...
    showMask() {
        ...
        if (Number(this.data.inputValue * this.data.number) >= 0) {
            if (
                Number(this.data.inputValue * this.data.number) >
            ...
            app.request(
                ...
                {
                    money: Number(this.data.inputValue * this.data.number),
                },
                (data, code) => {
                    if (code !== 200) {
